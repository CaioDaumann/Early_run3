...
def plott(data_hist,mc_hist,mc_rw_hist ,output_filename,xlabel, zmmg = None , postEE = True, endcap = False ):
    ...
    hep.histplot(
                mc_rw_hist,
                density=True,
                label=r'$Z\rightarrow ee$ - Simulation corrected',
                color = "None",
                linewidth=3,
                linestyle='--',
                ax=ax[0],
    ...
    # lets print the mean of the Data/MC agreement  
    if(  'mvaID' in xlabel ):
        
        print(xlabel, 'Is EE? - ', endcap )
        print('MvaID Data/MC before correction agreement: ', np.round(100*np.abs(np.mean(1-ratio[ratio != 0])),2) )
        print('MvaID Data/MC after correction agreement: ',  np.round(100*np.abs(np.mean(1-ratio_rw[ratio_rw != 0])),2)) 
        
        print('MvaID Data/MC before correction agreement (weighted): ',  np.round(100*np.abs(np.mean(1-ratio[ratio != 0])),2) )
        print('MvaID Data/MC after correction agreement  (weighted): ',  np.round(( (np.sum(mc_hist_rw_numpy[0][ratio_rw != 0])/integral_mc_rw) * 1/( np.sum(mc_hist_rw_numpy[0][ratio_rw != 0]) ))*100*np.abs(np.mean(1-mc_hist_rw_numpy[0][ratio_rw != 0]*ratio_rw[ratio_rw != 0])),2)) 
        
        
        #print(ratio[ratio != 0])
        #print(ratio_rw[ratio_rw != 0])
        #print(xlabel)
       
        #print('2: Data/MC before correction agreement: ', 100*(np.mean(np.abs(1-ratio[ratio != 0][:-1]))))
        #print('2: Data/MC after correction agreement: ',  100*(np.mean(np.abs(1-ratio_rw[ratio_rw != 0][:-1]))))     
        
        # Now weighted
        #print('rw: Data/MC before correction agreement: ', 100*(1/mc_hist_rw_numpy[0][ratio_rw != 0].sum())*(np.mean(np.abs(1-mc_hist_rw_numpy[0][ratio_rw != 0]*ratio_rw[ratio_rw != 0]))))
        #print('rw: Data/MC before correction agreement: ', 100*(1/mc_hist_numpy[0][ratio != 0].sum())*(np.mean(np.abs(1-mc_hist_numpy[0][ratio != 0]*ratio[ratio != 0]))))

        #integral_mc_rw_ = mc_rw_hist[ratio_rw != 0].sum().value * (mc_hist_rw_numpy[1][ratio_rw != 0][1] - mc_hist_rw_numpy[1][ratio_rw != 0][0])
        #print('Real buddy: ',  100*(1/ np.sum(mc_hist_rw_numpy[0][ratio_rw != 0] ) )*(np.mean( (mc_hist_rw_numpy[0][ratio_rw != 0] )*np.abs(1-ratio_rw[ratio_rw != 0]))))
        #print('Real buddy: ',  100*(1/ np.sum(mc_hist_numpy[0][ratio != 0] ) )*(np.mean( (mc_hist_numpy[0][ratio != 0] )*np.abs(1-ratio[ratio != 0]))))
        #print('Real buddy: ',  100*(np.mean(((mc_hist_numpy[0][ratio != 0]) / (mc_hist[ratio != 0].sum().value * (mc_hist_numpy[1][1][ratio != 0] - mc_hist_numpy[1][0][ratio != 0])))*np.abs(1-ratio[ratio != 0])))) 


    errors_nom = 5*(np.sqrt(data_hist_numpy[0])/integral_data) / ( (mc_hist_numpy[0] + 1e-15 ) / integral_mc)
    ...
    if( "mva" in str(xlabel)  ):
        xlabel_new = "Photon identification ID score"
    ...
    if( 'mva' in xlabel ):
        if( endcap ):
            ...
            ax[0].text(0.05, 0.95, r'EE photons', transform=ax[0].transAxes, fontsize=20, verticalalignment='top')
        else:
            ax[0].text(0.05, 0.95, r'EB photons', transform=ax[0].transAxes, fontsize=20, verticalalignment='top')
            #ax[0].text(0.05, 0.95, r'|$\eta$| < 1.442', transform=ax[0].transAxes, fontsize=20, verticalalignment='top')
    else:
        ax[0].text(0.05, 0.95, r'EB+EE photons', transform=ax[0].transAxes, fontsize=20, verticalalignment='top')
        #ax[0].text(0.05, 0.95, r'|$\eta$| < 2.5', transform=ax[0].transAxes, fontsize=20, verticalalignment='top')

    #mplhep.cms.text( loc = 1, ax = ax[0],  )
 
    hep.cms.label(data=True, ax=ax[0], loc=0, label = "Preliminary", com=13.6, lumi = 34.7)
